...
$(document).ready(function() {
  ...
  // お気に入り関連
  function addFavorite(currentUID, messageID, message) {
    var favorite = {
      message: message,
      createdAt: firebase.database.ServerValue.TIMESTAMP,
    };
    firebase.database().ref().child("favorites/" + currentUID + "/" + messageID).push(favorite);
  }
  ...
  $(".favorite-button").click(function() {
    alert("あらーと");
    // クリックした時このメッセージの星が黒いか白いか判断する
    var classVal = $('.favorite-button').attr('class');
    var id = $(this).attr('id');
    alert(id + ":" + classVal);
    if(classVal == "glyphicon glyphicon-star favorite-button") {
      // 黒い場合
      // 1. このメッセージをお気に入りから解除する
      deleteFavorite(current);
    // 2. 星を白い画像にセットする
    // (3. )
    } else {
      // 白い場合
